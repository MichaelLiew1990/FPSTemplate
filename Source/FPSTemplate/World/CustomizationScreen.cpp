	...
	: Super(ObjectInitializer)
{
	...
	ItemMesh->AttachParent = BackgroundMesh;

	Sight = ObjectInitializer.CreateDefaultSubobject<UStaticMeshComponent>(this, TEXT("Sight"));
	Sight->AttachParent = ItemMesh;
	Sight->AttachSocketName = FName("b_gun_Root");
	...
	Suppressor->AttachParent = ItemMesh;
	Suppressor->AttachSocketName = FName("Muzzle");
...
void ACustomizationScreen::GetItemBounds(FVector & OutOrigin, float & OutSphereRadius) const
{
	...
	{
		OutOrigin =  ItemMesh->SkeletalMesh->Bounds.Origin;
		OutSphereRadius = ItemMesh->SkeletalMesh->Bounds.SphereRadius;
	}
}
...
void ACustomizationScreen::UpdatePreview(const FLoadoutSlotStruct & LoadoutSlot)
{
	...
	if (Item)
	{
		...
		ItemMesh->SetAnimInstanceClass(Item->Mesh->AnimBlueprintGeneratedClass);
		...
		if (Item->SightComponent)
		{
			...
			{
				Sight->SetStaticMesh(Item->SightComponent->StaticMesh);
			}
		}
		else
		{
		...
		if (LoadoutSlot.bHasSuppressor && Firearm && Firearm->SuppressorComponent)
		{
			...
			Suppressor->SetStaticMesh(Firearm->SuppressorComponent->StaticMesh);
		}
		else
		{
		...
		// We have to reset all materials to their default manually because of a bug
		for (uint8 i = 0; i < ItemMesh->GetNumMaterials(); i++)
		{
			if (!bFoundMaterial || i != Item->PatternMaterialSlot)
			{
				ItemMesh->SetMaterial(i, Item->Mesh->GetMaterial(i));
			}
		}
	}
	else
