...
bool ULoadoutBuilderComponent::IsItemFromCurrentSlot(const FName & ItemName) const
{
	...
	if (ItemCDO)
	{
		...
		{
			if (CurrentItemSlot < EItemSlotEnum::Grenade - 1)
			{
				return false;
			}
		}
		else if (ItemCDO->ItemSlot - 1 != CurrentItemSlot)
		{
			return false;
		}
	}
	else if (CurrentItemSlot == EItemSlotEnum::Knife - 1)
	{
...
bool ULoadoutBuilderComponent::CanEquipSight(const FName & SightName) const
{
	...
	if (SightCDO)
	{
		...
		if (ItemCDO && ItemCDO->SightComponent && ItemCDO->SightComponent->bVisible)
		{
...
bool ULoadoutBuilderComponent::CanEquipSuppressor() const
{
	...
	if (ItemCDO)
	{
		return ItemCDO->SuppressorComponent->bVisible;
	}
