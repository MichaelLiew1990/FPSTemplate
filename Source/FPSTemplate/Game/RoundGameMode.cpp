...
void ARoundGameMode::InitGameState()
{
	...
	if (RoundGameState)
	{
		...
		// Warmup time
		RoundGameState->TimeRemaining = 5;
	}
}

ERoundStateEnum::Type ARoundGameMode::GetRoundState() const
{
	ARoundGameState * RoundGameState = Cast<ARoundGameState>(GameState);
	if (RoundGameState)
	{
...
bool ARoundGameMode::CanPlayerSpawn(ABasePlayerController * PC) const
{
	...
	{
		if (GetRoundState() == ERoundStateEnum::WarmUp)
		{
			return true;
		}
	...
	for (FConstPlayerControllerIterator PlayerItr = GetWorld()->GetPlayerControllerIterator(); PlayerItr; ++PlayerItr)
	{
		...
		if (PC)
		{
			if (PC->GetPawn())
			{
				...
				if (PlayerStart)
				{
					PC->GetPawn()->SetActorLocationAndRotation(PlayerStart->GetActorLocation(), PlayerStart->GetActorRotation());
				}
			}
			else
			{
