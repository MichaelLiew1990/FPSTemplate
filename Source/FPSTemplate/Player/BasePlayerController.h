...
class FPSTEMPLATE_API ABasePlayerController : public APlayerController
{
	...
	UPROPERTY(ReplicatedUsing = OnRep_State)
		TEnumAsByte<EPlayerStateEnum::Type> State;
...
public:
	...
	UPROPERTY(BlueprintReadOnly, Category = PlayerController)
		TEnumAsByte<EMessageTypeEnum::Type> CurrentMessageType;
	...
	// Return state
	EPlayerStateEnum::Type GetState() const { return State; };
...
private:
	...
	void BeginChatMessage(EMessageTypeEnum::Type MessageType);
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = PlayerController)
		void SetLoadout(const FLoadoutStruct & NewLoadout);

	// Called from game mode when custom loadout was evaluated
	void ApplyLoadout(const FLoadoutStruct & NewLoadout);

	const FLoadoutStruct & GetLoadout() const { return Loadout; };

	UFUNCTION(Server, Reliable, WithValidation)
		void ServerSetLoadout(const FLoadoutStruct & NewLoadout);
